...
public class ArcherAttack : MonoBehaviour
{
    ...
    private void Update()
    {
        if ((Input.GetMouseButtonDown(0) || Input.GetKeyDown(KeyCode.Return)) && cooldownTimer > attackCooldown && archerMovement.canAttack())
        {
            isCharging = true;
            chargeTimer = 0f;
        }
        ...
        if ((Input.GetMouseButtonUp(0) || Input.GetKeyUp(KeyCode.Return)) && isCharging && archerMovement.canAttack())
        {
            Attack();
            isCharging = false;
        }
